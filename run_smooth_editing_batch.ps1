...
foreach ($inFile in $inputFiles) {
    ...
    if ($needsCompression) {
        ...
        # Execute ffmpeg with ALL-INTRA (-g 1) flag
        & $ffmpeg -y -v warning -stats -i "`"$($inFile.FullName)`"" -c:v h264_nvenc -g 1 -preset p4 -cq 18 -pix_fmt yuv420p -c:a copy "`"$expectedOutputFile`""
